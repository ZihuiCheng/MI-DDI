...
df_drugs_smiles = pd.read_csv('data/biosnap3.csv')
...
smilesmat = {drug_id: mol for drug_id, mol in MOL_EDGE_LIST_FEAT_MTX.items() if mol is not None}
...
df_all_pos_ddi = pd.read_csv('data/biosnap44.csv')#newddixiao-1
...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
       ...
       # print(pos_h_samples)
        ...
        #hs = Batch.from_data_list(hs)
        #ts = Batch.from_data_list(ts)
    ...
    def __create_graph_data(self, id):
        ...
        return Data(x=features, edge_index=edge_index, edge_attr=edge_attr1, smiles=id)
...
class DrugDataset1(Dataset):
    def __init__(self, tri_list, ratio=1.0, neg_ent=1, disjoint_split=False, shuffle=True):
        ...
        #print(self.drug_ids)
        np.save('drug_ids.npy',self.drug_ids)
        if shuffle:
            random.shuffle(self.tri_list)
        limit = math.ceil(len(self.tri_list) * ratio)
        self.tri_list = self.tri_list[:limit]
    ...
    def collate_fn(self, batch):
        ...
        pos_tri = (drugs, drugs, pos_rels[0:645])
